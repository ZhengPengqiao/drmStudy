...
//绘制一帧全色的图，每次绘制更换一下颜色
void UserColorTest(unsigned char *dest, int stride, int w, int h)
{
    static int i = 0;
    ...
    printf("UserColorTest:draw\n");
}

//绘制一帧全色的图，每次绘制更换一下颜色
void ClearByColor(unsigned char *dest, int stride, int w, int h, int r, int g, int b)
{
    unsigned int j, k, off;
    ...
    for (j = 0; j < h; ++j)
    {
        ...
        {
            off = stride * j + k * 4;
            *(uint32_t*)&(dest[off]) = (r << 16) | (g << 8) | b;
        }
    }

    printf("ClearByColor:draw \n");
}
...
int main(int argc, char *argv[])
{
    ...
    unsigned char* buf = (unsigned char*)mmap(0, creq.size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, mreq.offset);
    if (buf == MAP_FAILED)
    {
        printf("mmap failed!\n");
    }
    ...
    printf("fd=%d   crtcid=%d framebuffer=%d\n", fd, crtcid, framebuffer);
    //一切准备完毕，只差连接在一起了！
    ...
    while (1)
    {
        if( uc ==1 )
        {
            UserColorTest(buf, stride, width, height);
            drmModePageFlip(fd, crtcid, framebuffer, DRM_MODE_PAGE_FLIP_EVENT, 0);
            sleep(2);    
        }
        else
        {
            ClearByColor(buf, stride, width, height, r, g, b);
            drmModePageFlip(fd, crtcid, framebuffer, DRM_MODE_PAGE_FLIP_EVENT, 0);
            sleep(2); 
        }
    }
